...
class TreeViewPDFPrinter
{
    ...
    public function generateHTML()
    {
        ...
        if (is_countable($this->records) ? count($this->records) === 0 : 0) {
    ...
    protected function generateRecordRows($record)
    {
        $html = '';
        $record_cells = '';
        $this->sugar_smarty->assign('record_level', is_countable($record['nodes_history']) ? count($record['nodes_history']) - 1 : -1);
        $this->sugar_smarty->assign('column_count', is_countable($this->columns) ? count($this->columns) : 0);
