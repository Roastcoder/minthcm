...
class TemplateEngine
{
     ...
     * @return array
     */
    public function getRecords($fieldIdArray, $fieldArray, $results)
    {
        if (is_countable($results) ? $results > 0 : 0) {
     ...
     * @return HTML
     */
    public function getFullHtml($variables, $fieldIdArray, $fieldArray, $results, $pdf_var = null)
    {
        ...
        if (null != $pdf_var) {
            foreach ($pdf_var as $pdf_var_key => $var) {
                $html = str_replace("\$$pdf_var_key", $var, $html);
            }
            //zamieniamy zmienne, które znalazy się z hooka
        }
        return $html;
    }
     ...
     */
    public function getPreview($vardefs)
    {
        global $timedate;
        $tpl = str_replace(array('<!--repeat-->', '<!--endrepeat-->'), array('<repeat>', '</repeat>'), $this->all_text);
        $repeat = strstr($tpl, "<repeat>");
     ...
     * @return TCPDF
     */
    public function getPDFHeader($report = null)
    {
     ...
     * @return Array
     */
    public function getFieldArray($thisReport)
    {
        ...
        foreach ($arrayList as $thisList) {
            if ('yes' == $thisList['display']) {
                ...
                $fieldArray[] = array(
                    'label' => $thisList['name'],
                    'width' => (isset($thisList['width']) && '' != $thisList['width'] && '0' != $thisList['width']) ? $thisList['width'] : '100',
