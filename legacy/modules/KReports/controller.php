...
class KReportsController extends SugarController
{
    ...
    public function action_BucketmanagerView()
    {
        $this->view = 'bucketmanager';
    }

    public function action_DListManagerView()
    {
        $this->view = 'dlistmanager';
    }

    public function action_check_access_level()
    {
    ...
    public function action_get_teamids()
    {
        global $db;
    ...
    public function action_get_securitygroups()
    {
        global $db;
    ...
    public function action_get_korgobjects()
    {
        global $db;
    ...
    public function action_get_autocompletevalues()
    {
        ...
        if ('' != $thisModule->table_name) {
            ...
            if ('field' == $fieldArray[0] && isset($thisModule->field_defs[$fieldArray[1]]) && 'id' != $fieldArray[1]) {
    ...
    public function action_getVisualizationPreview()
    {
    ...
    public function action_get_report_geocodes()
    {
    ...
    public function action_geocode_report_results()
    {
        $thisReport = new KReport();
        $thisReport->retrieve($_REQUEST['record']);
    ...
    public function action_getReportHtmlResults()
    {
        ...
        if (isset($_REQUEST['whereClause'])) {
            ...
            foreach ($repWhereClause as $repWhereClauseIndex => $repWhereClauseData) {
                if (isset($dashletWhereClause[$repWhereClauseData['fieldid']]) && 'KRignoreFilter' !== $dashletWhereClause[$repWhereClauseData['fieldid']]['value']) {
                    switch ($repWhereClause[$repWhereClauseIndex]['usereditable']) {
                        ...
                        case 'yo2':
                        case 'yo1':
                            if (true == $dashletWhereClause[$repWhereClauseData['fieldid']]['value']) {
                        ...
                        default:
                            switch ($repWhereClause[$repWhereClauseIndex]['type']) {
                                ...
                                case 'datetime':
                                    ...
                                    if ('ignore' == $repWhereClause[$repWhereClauseIndex]['operator']) {
                ...
                } elseif (isset($dashletWhereClause[$repWhereClauseData['fieldid']]) && 'KRignoreFilter' == $dashletWhereClause[$repWhereClauseData['fieldid']]['value']) {
    ...
    public function action_get_pushpinimages()
    {
    ...
    public function action_export_to_pdf($report_id = null, $path = null, $template_id = null)
    {
        ...
        if ('ktreeview' == $report_presentation_params['plugin']) {
            ...
            if (file_exists("modules/KReports/Plugins/Integration/kpdfexport/templates/{$_REQUEST['template_id']}.html")) {
                if ("Default" != $_REQUEST['template_id']) {
        ...
        (null != $path) ? $pdf->Output($path, 'F') : $pdf->Output(self::generateReportFileName($report_bean->name), 'D');
    }
    ...
    public function action_loadTreeViewNode()
    {
        ...
        if ('root' === $tree_view_generator->parent_node) {
