...
class simple_html_dom_node
{
    ...
    {
        $children_count = is_countable($this->children) ? count($this->children) : 0;
        if ($children_count > 0) {
            return $this->children[$children_count - 1];
        }
        return null;
    }
    ...
    public function find($selector, $idx = null)
    {
        ...
        if (is_null($idx)) {
            ...
            $found_count = is_countable($found) ? count($found) : 0;
            $idx = $found_count + $idx;
        }
...
class simple_html_dom
{
    ...
    public function prepare($str, $lowercase = true)
    {
        $this->clear();
        $this->doc = $str;
        $this->pos = 0;
    ...
    protected function remove_noise($pattern, $remove_tag = false)
    {
        ...
        for ($i = $count - 1; $i > -1; --$i) {
            $noise_count = is_countable($this->noise) ? count($this->noise) : 0;
            $key = '___noise___' . sprintf('% 3d', $noise_count + 100);
            $idx = ($remove_tag) ? 0 : 1;
