...
var postTa=document.createElement('textarea');postTa.name='current_post';postTa.value=document.MassUpdate.current_query_by_page.value;postTa.style.display='none';newForm.appendChild(postTa);document.MassUpdate.parentNode.appendChild(newForm);newForm.submit();return;}
else if(document.MassUpdate.massall.checked==true)
select=false;else
...
SUGAR.language.languages[module]=data;},get:function(module,str){if(typeof SUGAR.language.languages[module]=='undefined'||typeof SUGAR.language.languages[module][str]=='undefined'){console.warn('SUGAR.language: Missing Label: '+str+' in module: '+module);return str;}
...
async function open_popup(module_name,width,height,initial_filter,close_popup,hide_clear_button,popup_request_data,popup_mode,create,metadata,as_legacy=false){const isLegacyFilter=initial_filter&&!Array.isArray(initial_filter)
const shouldUseNewPopup=!as_legacy&&!isLegacyFilter
if(shouldUseNewPopup){const result=await window.LegacyEventManager.emit('OpenRelatePopup',{moduleName:module_name,fieldToNameArray:popup_request_data?.field_to_name_array,popupMode:popup_mode?.toLowerCase(),filterDefs:Array.isArray(initial_filter)?initial_filter:[],})
