...
class SugarWidgetSubPanelTopSelectButton_Delegations extends SugarWidgetSubPanelTopSelectButton
{
    ...
    public function display($widget_data, $additionalFormFields = null, $nonbutton = false)
    {
        ...
        if (ACLController::moduleSupportsACL($widget_data['module']) && !ACLController::checkAccess($widget_data['module'],
                'list', true)) {
        ...
        $initial_filter_fields = [
            'status_perm_advanced' => '["planned","worked","closed"]',
            'assigned_to_perm_advanced' => $this->parent_bean->assigned_user_id,
            'type_perm_advanced' => '["home","delegation"]',
        ];
        foreach ($initial_filter_fields as $alias => $value) {
        ...
        if (is_array($this->button_properties) && !empty($this->button_properties['add_to_passthru_data'])) {
            $popup_request_data['passthru_data'] = array_merge($popup_request_data['passthru_data'],
                $this->button_properties['add_to_passthru_data']);
        }
