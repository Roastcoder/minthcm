...
setMenuVisible(menu,left,top,false);}}};}();$(function(){$('.subPanelCountHint.incomplete').each(function(i,e){var module=$(e).attr('data-module');var subpanel=$(e).attr('data-subpanel');var record=$(e).attr('data-record');$.get('index.php?sugar_body_only=1&module='+module+'&subpanel='+subpanel+'&action=SubPanelViewer&inline=1&record='+record+'&layout_def_key=&inline=1&ajaxSubpanel=true&countOnly=1',function(resp,success){if(!success){console.error('Subpanel Row Count loading error');}else{$(e).html(resp);if(resp&&resp!='0'){$(e).closest('.collapsed').addClass('collapsed-override');}}});});});function acceptWorkSchedule(work_schedule_id){viewTools.GUI.statusBox.showStatus(SUGAR.language.get("app_strings","LBL_SAVING"),"info");viewTools.api.callController({module:"WorkSchedules",action:"acceptWorkSchedule",dataType:"json",async:false,dataPOST:{work_schedule_id:work_schedule_id,},callback:function(call_constroller_data){if(call_constroller_data==false||call_constroller_data==null){console.error(call_constroller_data);viewTools.GUI.statusBox.showStatus(SUGAR.language.get("app_strings","LBL_ERROR"),"error",3000);}else{location.reload();}},});}