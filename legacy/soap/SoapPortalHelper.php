...
$portal_modules = array('Contacts', 'Notes');
...
function portal_get_entry_list_limited($session, $module_name, $where, $order_by, $select_fields, $row_offset, $limit)
{
    ...
    if ($module_name == 'Cases') {
        ...
        //then call the methods that will load the cases related to the contact for this user
        if (!isset($_SESSION['viewable'][$module_name])) {
            //retrieve the contact id's for this user
            $c =get_contacts_in();
            if (!empty($c)) {
        ...
        //then call the methods that will load the bugs related to the contact for this user
        if (!isset($_SESSION['viewable'][$module_name])) {
...
$valid_modules_for_contact = array('Contacts'=>1, 'Cases'=>1, 'Notes'=>1, 'Bugs'=>1, 'KBDocuments'=>1);
